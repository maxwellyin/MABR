...
from utils.util import ReverseLayerF, NUM_LABELS, MODEL_CHECKPOINT, BiasDetector, DomainClassifier, DATA_NAME
import wandb
...
def validate_model(model, bias_detectors, domain_classifiers, validation_loader, criterion, device, threshold_high, threshold_low):
    ...
    domain_loss_protected_high_total, domain_loss_protected_low_total, domain_loss_unprotected_total = 0, 0, 0
    total_bias_samples_high, total_bias_samples_low = 0, 0
    ...
    with torch.no_grad():
        for batch in tqdm(validation_loader):
            ...
            bias_loss_total = 0
            domain_loss_total = torch.tensor(0.0, device=device)  # Initialize as tensor
            for l, (intermediate_output, bias_detector, domain_classifier) in enumerate(zip(intermediate_outputs, bias_detectors, domain_classifiers), start=1):
                bias_preds = bias_detector(intermediate_output)
                bias_loss = criterion[1](bias_preds.squeeze(), correct)
                bias_loss_total += bias_loss

                domain_loss = apply_domain_classification(intermediate_output, bias_preds, predictions, domain_classifier, batch['labels'], device, threshold_high, threshold_low)
                domain_loss_total += domain_loss  # Accumulate domain losses
...
def apply_domain_classification(intermediate_output, bias_preds, predictions, domain_classifier, labels, device, threshold_high, threshold_low):
...
for epoch in range(epochs):
    ...
    train_model(model, bias_detectors, domain_classifiers, train_loader, optimizer_main, optimizers_bias, optimizers_domain, scheduler_main, schedulers_bias, schedulers_domain, criterion, device, threshold_high, threshold_low)
    validate_model(model, bias_detectors, domain_classifiers, validation_loader, criterion, device, threshold_high, threshold_low)
